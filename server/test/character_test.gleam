...
pub fn standing_character_walks_in_open_space_test() {
  let plan = mockingbird()
  // Mid-body row 10 is walkable x=3..10; plenty of room east of (4.5, 10.5).
...
pub fn diagonal_input_is_normalized_to_walk_speed_test() {
  ...
  let c = standing_at(4.5, 10.5) |> character.set_move(1.0, 1.0)
  let after = character.step(c, plan)
  let dx = after.x -. c.x
  let dy = after.y -. c.y
...
pub fn between_level_switches_deck_via_stairs_test() {
  ...
  let c = standing_lower(6.5, 22.45) |> character.set_move(0.0, -1.0)
  let climbed = run_ticks(c, plan, 40)
  assert climbed.y <. 22.0
  assert climbed.deck == deckplan.Upper
}

pub fn upper_walker_descends_to_lower_via_stairs_test() {
  let plan = mockingbird()
  // Upper walker on 'B' steps onto the port 'L' half-flight at (5, 21)
  // and becomes Lower.
  let c = standing_at(5.5, 22.45) |> character.set_move(0.0, -1.0)
  let descended = run_ticks(c, plan, 40)
  assert descended.y <. 22.0
  assert descended.deck == deckplan.Lower
}

pub fn upper_walker_cannot_use_lower_stair_off_b_test() {
  let plan = mockingbird()
  // NOT standing on 'B': an upper walker on the 'U' stair (6, 21) cannot
  // sidestep onto the 'L' stair (5, 21).
  let c = standing_at(6.5, 21.5) |> character.set_move(-1.0, 0.0)
...
pub fn is_at_helm_false_when_seated_at_non_helm_console_test() {
  let plan = mockingbird()
  let c = Character(..standing_lower(5.5, 21.5), seat: Some("cargo_main"))
  assert !character.is_at_helm(c, plan)
}
...
pub fn try_sit_wrong_deck_is_too_far_test() {
  ...
  let assert Ok(seated) =
    character.try_sit(standing_lower(5.5, 22.5), plan, "cargo_main", False)
