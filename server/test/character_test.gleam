...
fn run_ticks(character: Character, class: ShipClass, n: Int) -> Character {
  case n {
    0 -> character
    _ -> run_ticks(character.step(character, class), class, n - 1)
  }
}
...
pub fn seated_character_ignores_move_input_test() {
  ...
  let after = run_ticks(c, class, 60)
  assert after.x == c.x
  assert after.y == c.y
}
...
pub fn standing_character_walks_in_open_space_test() {
  ...
  let after = character.step(c, class)
...
pub fn walking_straight_into_wall_stops_test() {
  ...
  let settled = run_ticks(c, class, 300)
  ...
  let one_more = character.step(settled, class)
...
pub fn walking_diagonally_into_wall_slides_test() {
  ...
  let soon = run_ticks(c, class, 20)
  assert close(soon.y, 2.3, 0.05)
  assert soon.x <. 4.0
  let later = run_ticks(soon, class, 15)
...
pub fn character_never_enters_non_walkable_tile_test() {
  ...
  let settled = run_ticks(c, class, 120)
  assert settled.x >. 0.0
  assert shipclass.is_walkable(
    class,
...
pub fn spawn_seated_at_helm_is_seated_at_helm_main_test() {
  let class = sparrow()
  let c = character.spawn_seated_at_helm(1, "ada", 7, class)
  assert c.seat == Some("helm_main")
  assert c.ship_id == 7
  assert c.x == 1.5
  assert c.y == 2.5
  assert character.is_at_helm(c, class)
}

pub fn spawn_at_spawn_tile_is_standing_at_spawn_test() {
  let class = sparrow()
  let c = character.spawn_at_spawn_tile(2, "grace", 7, class)
  assert c.seat == None
  assert c.ship_id == 7
...
pub fn try_sit_too_far_test() {
  ...
  assert character.try_sit(c, class, "helm_main", False) == Error("too_far")
}

pub fn try_sit_unknown_console_test() {
  let class = sparrow()
  let c = standing_at(1.5, 2.5)
  assert character.try_sit(c, class, "nonexistent", False)
...
pub fn try_sit_already_seated_test() {
  ...
  assert character.try_sit(c, class, "cargo_main", False)
